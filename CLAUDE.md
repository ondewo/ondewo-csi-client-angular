...
```text
Release ONDEWO CSI Angular Client ${ONDEWO_CSI_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO CSI Angular Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO CSI Angular Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/csi-client-angular`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/csi-client-angular versions --json
```
